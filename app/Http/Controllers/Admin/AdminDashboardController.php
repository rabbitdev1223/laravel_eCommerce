...
class AdminDashboardController extends Controller
{
    public function __invoke(Request $request )
    {
        ...
        $total_orders = Order::select(DB::raw('COUNT(*) as total'))->get()->first()->total;
        
        $total_users_not_super = User::select(DB::raw('COUNT(*) as total'))->whereExists(function($query){
        ...
        $date_ini = Carbon::today()->firstOfMonth()->setTime(0, 0, 0);
        $date_end = Carbon::today()->LastOfMonth()->setTime(23, 59, 59);
        
        $transaction = Transaction::whereBetween('created_at',[$date_ini, $date_end])->get();
        ...
        ->select(
            ...
            return view('admin.dashboard.dashboard', compact('total_orders', 'transaction','total_users_not_super','total_location', 'best_location', 'total_week_location', 'total_users', 'best_user', 'total_week_user', 'total_items_product', 'product_best_sale', 'total_week_product', 'initial_dt', 'final_dt', 'diffInDays', 'table1', 'table2', 'table3', 'table4', 'title1', 'title2', 'title3', 'title4'));
    }
}
